...
# - __fzf_complete_git_source_status
# - __fzf_complete_git_source_ls_files
# - __fzf_complete_git_source_staged
# - __fzf_complete_git_source_modified
# - __fzf_complete_git_source_log
# - __fzf_complete_git_source_branch
# - __fzf_complete_git_source_remote_branch
# - __fzf_complete_git_source_tag
# - __fzf_complete_git_source_reflog
# - __fzf_complete_git_source_stash
# - __fzf_complete_git_source_remote
# - __fzf_complete_git_source_author
#
# Note: __fzf_complete_git_source_switch_branch is tested in test_git_switch_branch_filter.fish
...
@test "source_status: returns output when there are modified files" (
    ...
    set -l output (__fzf_complete_git_source_status)
    _cleanup_test_repo
    test -n "$output"
) $status -eq 0
...
@test "source_status: shows staged files" (
    ...
    git add newfile.txt
    set -l output (__fzf_complete_git_source_status | string replace -ra $ANSI_ESCAPE '')
...
@test "source_ls_files: lists tracked files (null-separated)" (
    ...
    set -l output (__fzf_complete_git_source_ls_files | string split0)
...
@test "source_ls_files: returns empty in repo without tracked files" (
    ...
    set -l output (__fzf_complete_git_source_ls_files)
    _cleanup_test_repo
    test -z "$output"
) $status -eq 0
...
@test "source_staged: lists staged files when files are staged" (
    ...
    git add staged.txt
    set -l output (__fzf_complete_git_source_staged | string split0)
...
@test "source_staged: returns empty when nothing staged" (
    _setup_test_repo
    set -l output (__fzf_complete_git_source_staged)
    _cleanup_test_repo
    test -z "$output"
) $status -eq 0
...
@test "source_modified: lists modified files when files are modified" (
    ...
    set -l output (__fzf_complete_git_source_modified | string split0)
...
@test "source_modified: returns empty when no modifications" (
    _setup_test_repo
    set -l output (__fzf_complete_git_source_modified)
    _cleanup_test_repo
    test -z "$output"
) $status -eq 0
...
@test "source_log: returns commit entries with [commit] prefix" (
    _setup_test_repo
    set -l output (__fzf_complete_git_source_log | string replace -ra $ANSI_ESCAPE '')
...
@test "source_log: shows multiple commits" (
    ...
    set -l line_count (__fzf_complete_git_source_log | string replace -ra $ANSI_ESCAPE '' | wc -l | string trim)
    _cleanup_test_repo
...
@test "source_branch: lists local branches with [branch] prefix" (
    ...
    set -l output (__fzf_complete_git_source_branch | string replace -ra $ANSI_ESCAPE '')
...
@test "source_remote_branch: lists only remote branches with [remote] prefix" (
    ...
    set -l output (__fzf_complete_git_source_remote_branch | string replace -ra $ANSI_ESCAPE '')
...
@test "source_remote_branch: returns empty when no remotes" (
    _setup_test_repo
    set -l output (__fzf_complete_git_source_remote_branch)
    _cleanup_test_repo
    test -z "$output"
) $status -eq 0
...
@test "source_tag: lists tags with [tag] prefix" (
    _setup_test_repo
    git tag v1.0.0
    set -l output (__fzf_complete_git_source_tag | string replace -ra $ANSI_ESCAPE '')
...
@test "source_tag: returns empty when no tags" (
    _setup_test_repo
    set -l output (__fzf_complete_git_source_tag)
    _cleanup_test_repo
    test -z "$output"
) $status -eq 0
...
@test "source_reflog: returns reflog entries with [reflog] prefix" (
    _setup_test_repo
    set -l output (__fzf_complete_git_source_reflog | string replace -ra $ANSI_ESCAPE '')
...
@test "source_reflog: shows multiple entries after commits" (
    ...
    set -l line_count (__fzf_complete_git_source_reflog | string replace -ra $ANSI_ESCAPE '' | wc -l | string trim)
    _cleanup_test_repo
    test $line_count -ge 3
) $status -eq 0
...
@test "source_stash: returns stash entries when stashes exist" (
    ...
    set -l output (__fzf_complete_git_source_stash | string replace -ra $ANSI_ESCAPE '')
...
@test "source_stash: returns empty when no stashes" (
    _setup_test_repo
    set -l output (__fzf_complete_git_source_stash)
    _cleanup_test_repo
    test -z "$output"
) $status -eq 0
...
@test "source_remote: lists remotes" (
    ...
    set -l output (__fzf_complete_git_source_remote)
    _cleanup_test_repo
    test "$output" = origin
) $status -eq 0
...
@test "source_author: returns unique authors from log" (
    _setup_test_repo
    set -l output (__fzf_complete_git_source_author)
    _cleanup_test_repo
    test -n "$output"
) $status -eq 0
...
@test "source_author: returns unique authors only" (
    ...
    git commit --allow-empty -m "second" --quiet
    git commit --allow-empty -m "third" --quiet
    set -l count (__fzf_complete_git_source_author | wc -l | string trim)
    _cleanup_test_repo
